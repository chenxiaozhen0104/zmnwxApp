    ...
    <div class="container">
        ...
        <div class="deviceDetail-category">
            <div class="deviceDetail-serach">
                <div class="import">
                    <input class="deviceDetail-text" type="text" placeholder="请输入客户名称" v-model="val" />
                    <div class="clear-box" @click="clearContent" v-if="val.length">
                        <text class="clear">清空</text>
                    </div>
                </div>
            </div>
            <div class="deviceDetail-content" v-if="!val.length&&saves.length">
                <!--历史记录-->
                <div class="history">
                    <!--历史头部-->
                    ...
                    <div class="history-content">
                        <div class="history-details-content">
                            ...
                            <div class="history-record" v-for="item in saves">
        ...
        methods: {
            ...
            saveChange() {
                ...
                this._post("/api/Common/UpdateFieldValue",
                    { table: 'device', fileName: 'UseCompanyName', fileValue: this.val, byField: 'DeviceId', byFieldValue: this.id },
                    res => {
            ...
            clearHistory() {
                modal.confirm({
                    ...
                    if (value == "确认") {
                        this.saves = [];
                        storage.removeItem('save', event => { })
                    }
                })
            },
            saveHistory() {
                if (this.saves.some(m => m == this.val)) {
                    return;
                }
                //更新历史记录值
                this.saves.unshift(this.val)
                this.saves.length = this.saves.length > 8 ? 8 : this.saves.length
                storage.setItem('save', JSON.stringify(this.saves), event => {
                    this.save += this.val;
                    console.log('success' + this.save)
                })
            },
        },
        created() {
            storage.getItem('save', event => {
                if (event.result == "success") {
                    this.saves = JSON.parse(event.data)
                }
            })
        }
    }
