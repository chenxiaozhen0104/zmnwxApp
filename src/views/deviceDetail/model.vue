    ...
    <div class="container">
        ...
        <div class="deviceDetail-category">
            <div class="deviceDetail-serach">
                <div class="import">
                    <input class="deviceDetail-text" type="text" placeholder="请输入设备型号" v-model="val" />
                    <div class="clear-box" @click="clearContent" v-if="val.length">
                        <text class="clear">清空</text>
                    </div>
                </div>
            </div>
            <div class="deviceDetail-content" v-if="!val.length && modelList.length">
                <!--历史记录-->
                <div class="history">
                    <!--历史头部-->
                    ...
                    <div class="history-content">
                        <div class="history-details-content">
                            ...
                            <div class="history-record" v-for="item in modelList">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            saveChange() {
                ...
                this._post("/api/Common/UpdateFieldValue",
                    { table: 'device', fileName: 'Model', fileValue: this.val, byField: 'DeviceId', byFieldValue: this.id },
                    res => {
            ...
            clearHistory() {
                modal.confirm({
                    ...
                    if (value == "确认") {
                        this.modelList = [];
                        storage.removeItem('model', event => { })
                    }
                })
            },
            saveHistory() {
                if (this.modelList.some(m => m == this.val)) {
                    return;
                }
                //更新历史记录值
                this.modelList.unshift(this.val)
                this.modelList.length = this.modelList.length > 8 ? 8 : this.modelList.length;
                storage.setItem('model', JSON.stringify(this.modelList), event => {
                    this.model += this.val;
        ...
        created() {
            storage.getItem('model', event => {
                if (event.result == "success") {
                    this.modelList = JSON.parse(event.data)
                }
            })
        }
    }
